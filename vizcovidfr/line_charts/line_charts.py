...
def vactypedoses(vaccine_type):
    ...
    :param vaccine_type: the vaccine type we want to display.
        Should be either Pfizer, or Moderna, or AstraZeneca.
    :type vaccine_type: string
    :return: an animated line chart representing the actual 
    dose number or the actual cdu (common dispensing units) 
    number, of the chosen vaccine type (in storage).
    '''
    df_Vac_type = pd.read_csv(path_target, sep = ",")
    df_Vac_type = df_Vac_type.groupby(['type_de_vaccin'])
    pfizer = df_Vac_type.get_group('Pfizer').reset_index(drop = True)
    mdn = df_Vac_type.get_group('Moderna').reset_index(drop = True)
    astra = df_Vac_type.get_group('AstraZeneca').reset_index(drop = True)
    ...
    fig = px.line(df, x = 'date', y = ['nb_doses', 'nb_ucd'], title = 'Vaccine storage per type')
    fig.show()
#Rather choosing the variables ('nb_doses' and 'nb_ucd') 
#as arguments or in the animation?

#how to display the year of each date?
#how can I fix the title according to the chosen vaccine_type?
